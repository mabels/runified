...
func TestAppStartStop(t *testing.T) {
	...
	app := NewApp(&types.AppParam{Log: &log, CLIconfig: cliCFG}).(*app)
	app.httpHandler.RegisterHandler("/test", BindAppToHandler(app, func(app *types.AppHandler) {
		r := app.Request()
		w := app.Response()
...
func TestNewAppContext(t *testing.T) {
	...
	cliCFG := FromCommandLine([]string{})
	app := NewApp(&types.AppParam{Log: &log, CLIconfig: cliCFG})
	...
	app = NewApp(&types.AppParam{Log: &log, CLIconfig: cliCFG, Ctx: ctx})
